...
angular.module('Coursa', [
    ...
    'ngAria', 'ngAnimate', 'ngCookies', 'ngTouch', 'ngSanitize', 'ngMessages', 'ngMaterial','ngMap','ngTable','ngMessages','ui.bootstrap','blockUI',
    // inject extra 3rd party angular modules
    'ui.router', 'pascalprecht.translate', 'LocalStorageModule', 'googlechart', 'chart.js', 'angularMoment', 'uiGmapgoogle-maps', 'md.data.table','gridster','daterangepicker','datePicker',
    // UI grid
...
.constant('API_CONFIG', {
    'url':  'http://Coursa-api.invensense.com/'
})
/**
 *  SETUP TRANSLATIONS
 */
.config(function ($stateProvider, $urlRouterProvider, $translateProvider, $translatePartialLoaderProvider, localStorageServiceProvider, APP) {
    ...
    .state('404', {
        ...
        controller: function($scope, $state, APP) {
            ...
            $scope.goHome = function() {
                $state.go('admin-panel.default.store-analytics');
            };
        }
    })
    ...
    $urlRouterProvider.when('', '/stores/analytics');
    $urlRouterProvider.when('/', '/stores/analytics');
